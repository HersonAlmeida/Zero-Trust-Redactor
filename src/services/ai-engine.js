...
/**
 * Query the Python backend for accurate system hardware specs.
 * Unlike the browser-only detectHardware() which is limited by navigator.deviceMemory,
 * this uses psutil + wmic on the server for real RAM and GPU data.
 *
 * Returns: { ram_gb, gpu: {name,vendor,has_dedicated_gpu},
 *            has_enough_ram, deep_ai_available, recommended_mode, os }
 *
 * Falls back gracefully to a safe "fast-only" profile if the server is down.
 */
export async function checkHardwareCapabilities() {
  const apiUrl = import.meta.env.DEV ? '/api/system/hardware' : '/system/hardware';
  try {
    ...
    logDebug('⚠️ Hardware check failed, defaulting to Fast Mode:', err.message);
    return {
      ram_gb: 0,
...
/**
 * Clear browser-cached model data from IndexedDB
 */
